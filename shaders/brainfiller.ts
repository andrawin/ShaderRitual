...
/*
 * "Brainfiller" — a quadtree-subdivided path tracer flying through a city of
 * spheres, holograms and glitch screens. The heaviest shader in the set.
 *
 * This one is GLSL ES 3.00 (it needs uint / floatBitsToUint / bit ops), so it
 * runs through the engine's `glsl3` path. It's also a progressive accumulator:
 * each frame blends a few fresh samples with its previous frame via iChannel0
 * (the ping-pong feedback).
 *
 * Port notes vs the Shadertoy original:
 *   - wrapped for our pipeline (in vec2 vUv / out vec4; mainImage is `inout`
 *     so the `fragColor *= 0.0` clear works)
 *   - the BPM timeline (`beat`) is LOOPED so the piece never fades permanently
 *     to black the way the original's one-shot intro/outro does
 *   - SAMPLES_PER_FRAME trimmed for real-time (feedback denoises the rest)
 *   - one constructor that relied on a vec/scalar quirk made explicit (.x).
 *
 * Elements:
 *   exposure -> overall brightness
 *   neon     -> emissive (holograms / glitch screens) intensity
 *   trails   -> progressive-accumulation persistence
 */
...
uniform float exposure_react;
uniform float exposure_visible;
uniform float neon_react;
uniform float neon_visible;
...
void mainImage(inout vec4 fragColor, in vec2 fragCoord){
  ...
  p *= rotate2D(iCamOrbit * 0.5); // camera roll

  vec3 seed = hash3f(vec3(p, iTime));
  float time = iTime;
  // looped BPM timeline so the piece never fades permanently to black
  float beat = 16.0 + mod(iTime * 2.25, 272.0);
  ...
  for(int i = 0; i ++ < SAMPLES_PER_FRAME;){
    ...
    for(int i = 0; i ++ < TRAVERSAL_STEPS;){
...

      {
...

        {
          ...
          if(cell.z != 0.0){
            ...
            vec3 rotSphere = bvOrRot - vec3(0.0, 0.0, 4.0 + 0.5 * cellSize.x);
            isect2 = isectSphere(
              rotSphere,
              rd,
            ...
            if(isect2.w < isect.w){
              ...
              vec2 guv = 0.5 + ((coord / (cellSize - 0.008)).xy);
              vec3 grafix =
                ...
                cellDice.y < 0.4 ? saturate(abs(mod(6.0 * (guv.y + time + cellDice.z) + vec3(0, 4, 2), 6.0) - 3.0) - 1.0) :
      ...
      fragColor.xyz += colRem * i_emissive * (1.0 + neon_react * 2.0);
        ...
        if(spec > 0.0){
          ...
          colRem *= max(
            i_Fh / mix(Fn, 1.0, i_metallic)
              / (NdotV * (1.0 - halfSqRoughness) + halfSqRoughness)
              * NdotL / (NdotL * (1.0 - halfSqRoughness) + halfSqRoughness)
              * i_VdotH / i_NdotH,
            0.0
          );
  ...
  float fbk = trails_visible * clamp(0.5 + trails_react * 0.4, 0.0, 0.95);
  fragColor = mix(
  ...
  fragColor.xyz *= (1.0 + exposure_react * 1.5);
}
void main(){ vec4 c = vec4(0.0); mainImage(c, vUv * iResolution.xy); outColor = c; }
`;
...
export const brainfiller: ShaderDef = {
  id: 'brainfiller',
  name: 'Brainfiller',
  description:
    'A quadtree path tracer flying through a city of spheres, holograms and glitch screens. GLSL ES 3.00, progressive — heavy, give your GPU a moment.',
  glsl3: true,
  bufferShader,
  imageShader,
  elements: [
    {
      id: 'exposure',
      name: 'Exposure',
      description: 'Overall brightness of the render. React pumps it.',
      defaultBand: 'high',
      defaultAmount: 1.0,
      canHide: false,
      defaultVisible: true,
    },
    {
      id: 'neon',
      name: 'Neon',
      description: 'Emissive holograms + glitch screens. React drives their glow.',
      defaultBand: 'low',
      defaultAmount: 1.5,
      canHide: false,
      defaultVisible: true,
    },
    {
      id: 'trails',
      name: 'Accumulation',
      description: 'Progressive feedback persistence. Higher = smoother but smearier.',
      defaultBand: 'mid',
      defaultAmount: 0.6,
      canHide: true,
      defaultVisible: true,
