...
/*
 * "Siren" — a writhing organic blob whose surface is a tiny SIREN neural network
 * baked into the SDF, over a starfield. CC0, by Blackle Mori.
 *
 * The original inflated the blob from a music-FFT texture; here `snd` comes from
 * the engine's band-driven elements (pulse / colour / glow). Ported to GLSL-ES
 * 1.00: texelFetch FFT removed, mouse removed, compound loop condition split.
 * The mat4 weight network is valid ES 1.00 as-is.
 */
...
void camera(vec2 fragCoord, out vec3 ro, out vec3 rd, out mat3 t){
  ...
  float ff = sin(iTime / 20.0) + iCamOrbit;
  vec2 m = PI * ff + vec2(0.1 / iResolution.xy) * (PI * 2.0);
  m.y = sin(m.y * 0.5) * 0.3 + 0.5;
  vec2 sm = sin(m) * (1.0 + sin(iTime / 10.0) / 2.0), cm = cos(m);
...
vec3 erot(vec3 p, vec3 ax, float ro){ return mix(dot(p, ax) * ax, p, cos(ro)) + sin(ro) * cross(ax, p); }

float scene(vec3 p){
  if(length(p) > 1.0) return length(p) - 0.8;
  p /= 1.0 + snd;
...
vec3 palette(float t){
  if(t < 1.0) t += 1.0;
  vec3 a = vec3(0.5), b = vec3(0.5), c = vec3(1.0), d = vec3(0.563, 0.416, 0.457 + 0.2);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
  snd = clamp(pulse_react, 0.0, 1.0) * 0.4;
  ...
  vec3 init = vec3(-3.0, 0.0, 0.0);
  rd.x += sin(iTime / 1000.0) * 2.0;

  float yrot = 0.5;
  float zrot = iTime * 1.42 + iCamOrbit;
  ...
  vec3 nz = p - erot(p, vec3(1), 2.0) + erot(p, vec3(1), 4.0);
  ...
  vec3 bg = stars(rd) * (1.0 + 30.0 * snd * (0.3 + glow_react));

  fragColor = vec4(hit ? col * palette(snd * 2.0 + color_react + sin(iTime / 10.0)) * 2.0 : bg, 1.0);
  fragColor = smoothstep(-0.02, 1.05, sqrt(fragColor)) * (1.0 - dot(uv, uv) * 0.5);
}
...
export const siren: ShaderDef = {
  id: 'siren',
  name: 'Siren',
  description: 'A writhing organic blob whose surface is a tiny neural network, over a starfield (CC0, Blackle Mori).',
  bufferShader,
  imageShader,
  elements: [
    { id: 'pulse', name: 'Pulse', description: 'Inflates + brightens the blob. React on bass.', defaultBand: 'low', defaultAmount: 1.0, canHide: false, defaultVisible: true },
    { id: 'color', name: 'Colour', description: 'Shifts the surface palette. React on mids.', defaultBand: 'mid', defaultAmount: 0.6, canHide: false, defaultVisible: true },
    { id: 'glow', name: 'Stars', description: 'Brightness of the starfield. React on treble.', defaultBand: 'high', defaultAmount: 1.0, canHide: false, defaultVisible: true },
  ],
};
